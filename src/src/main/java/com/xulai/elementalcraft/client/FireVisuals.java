 ...
 * 采用“动态扇面”流动刀光设计。全等级固定2格距离，扇面角度宽度随等级变化(25%-100%)。
 ...
 * Uses a "Dynamic Sector" flowing blade light design. Fixed 2-block distance, sector angle width scales with Tier (25%-100%).
...
public class FireVisuals {
    ...
    public static class ClientEvents {
        ...
        private static void playFlowingMeleeSwing(LivingEntity entity, int tier) {
            ...
            // 挥砍半径固定为 2.0 (全等级一致)
            // Slash radius fixed at 2.0 (Consistent across all tiers)
            double radius = 2.0; 
            
            // 挥砍中心点 (眼部位置)
            // Slash center point (Eye position)
            Vec3 centerPos = entity.getEyePosition();

            // 1. 生成核心横扫粒子 (固定在 1.5 格)
            // 1. Spawn core SWEEP_ATTACK particle (Fixed at 1.5 blocks)
            Vec3 sweepPos = centerPos.add(look.scale(1.5)); 
            level.addParticle(ParticleTypes.SWEEP_ATTACK, 
                    sweepPos.x, sweepPos.y, sweepPos.z, 
                    look.x, look.y, look.z);

            // 2. 生成半月形火焰轨迹
            // 2. Spawn crescent flame trail
            
            // 基础总角度：45度 (+22.5 ~ -22.5)
            // Base total angle: 45 degrees (+22.5 ~ -22.5)
            double baseTotalAngle = Math.toRadians(45); 
            
            // 根据等级计算实际显示的弧长比例
            // Tier 1: 25%, Tier 2: 50%, Tier 3: 75%, Tier 4: 100%
            // Calculate actual arc length ratio based on tier
            double angleMultiplier = tier * 0.25; 
            double actualAngle = baseTotalAngle * angleMultiplier;

            // 计算起始和结束角度（以视线为中心对称分布）
            // Calculate start and end angles (Symmetrically distributed around look vector)
            double startAngle = actualAngle / 2.0;  // 右侧 / Right
            double endAngle = -actualAngle / 2.0;   // 左侧 / Left

            // 粒子数量：随弧长比例动态调整，保证密度一致
            // Particle count: Dynamically adjusted with arc length to ensure consistent density
            ...
            for (int i = 0; i <= particleCount; i++) {
                ...
                // 几何构建：水平圆弧
                // Geometry Construction: Horizontal Arc
                Vec3 horizontalOffset = right.scale(Math.sin(angle) * radius);
                // 向前推一点 (Cos函数 * 0.8 压扁系数)
                // Push forward slightly (Cos function * 0.8 flattening factor)
                ...
                // === 跑马灯视觉模拟 ===
                // 掉落速度曲线：右边掉得很快，左边完全不动
                // === Marquee Simulation ===
                // Drop speed curve: Right side drops fast, Left side stays still
                ...
                // 生成主火焰粒子
                // Spawn main flame particle
                level.addParticle(flameType, 
                        p.x, p.y, p.z, 
                        0, dropVelocity, 0);

                // Tier 3+ 特效 (熔岩颗粒)
                // 仅在左侧生成
                // Tier 3+ FX (Lava particles)
                // Only spawn on the left side
                if (tier >= 3 && progress > 0.7) {
                     level.addParticle(ParticleTypes.LAVA, p.x, p.y, p.z, 0, 0, 0);
                }

                // Tier 4 特效 (末端高亮)
                // Tier 4 FX (Tip highlight)
    ...
    private static void playTieredImpactExplosion(Entity target, int tier) {
        ...
        // [Feature] 1-2 seconds visual residue
        // Tier 4 (Legendary) or Tier 3 (Epic) triggers this
        if (tier >= 3) {
            // 生成一团速度极低（几乎悬停）的灵魂火/火焰粒子
            // 粒子的生命周期通常在 20-40 tick (1-2秒) 左右
            // Spawn a cluster of very low speed (almost hovering) Soul Fire/Flame particles
            // Particle lifetime is usually around 20-40 ticks (1-2 seconds)
            serverLevel.sendParticles(ParticleTypes.SOUL_FIRE_FLAME,
                    target.getX(), target.getY() + target.getBbHeight() * 0.6, target.getZ(),
                    15, 0.4, 0.4, 0.4, 0.005); // Speed very low, makes particles hover
            
            // 再加一点烟雾增加持续感
            // Add some smoke for duration feel
