...
public class FrostSnowflakeParticle extends TextureSheetParticle {
    ...
    public void tick() {
        ...
        if (this.age < this.fadeInTicks) {
            float t = (float) this.age / this.fadeInTicks;
            this.alpha = t;
            this.quadSize = this.startSize * t;
        } else if (this.age >= this.lifetime - this.fadeOutTicks) {
            float t = (float) (this.lifetime - this.age) / this.fadeOutTicks;
            this.alpha = Math.max(0f, t);
            this.quadSize = this.startSize * Math.max(0.2f, t);
        } else {
            this.alpha = 1.0f;
