 ...
 * 效果：
 * 1. 动态减伤：火属性伤害减免取决于目标的潮湿层数 (读取配置文件)，不触发额外二次减伤。
 ...
 * Effects:
 * 1. Dynamic Damage Reduction: Fire damage reduction depends on target's wetness level (Reads from config), no secondary reduction.
...
public class SteamReactionHandler {
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (isFireDamage) {
            // 触发反应，传递当前层数
            // Trigger reaction, passing current level
            triggerSteamReaction(target, event, wetnessLevel);
        }
    }
     ...
     * Includes dynamic damage reduction calculation (Based on config).
     *
     * @param target 受击实体 / The victim entity
     * @param event 受伤事件 / The hurt event
     * @param wetnessLevel 目标的潮湿层数 / The target's wetness level
     */
    private static void triggerSteamReaction(LivingEntity target, LivingHurtEvent event, int wetnessLevel) {
        // A. 动态伤害衰减 (仅基于潮湿层数)
        // A. Dynamic Damage Reduction (Based on wetness level only)
        float originalDamage = event.getAmount();
        
        // 读取配置：单层减免比例 (默认为 0.1 / 10%)
        // Read Config: Reduction per level
        double reductionPerLevel = ElementalReactionConfig.wetnessFireReduction;
        
        // 读取配置：最大减免上限 (默认为 0.9 / 90%)
        // Read Config: Max reduction cap
        double maxReduction = ElementalReactionConfig.steamMaxReduction;

        // 计算减伤比例：Min(层数 * 单层比例, 上限)
        // Calculate reduction ratio: Min(Level * PerLevelRatio, Cap)
        float reductionRatio = (float) Math.min(wetnessLevel * reductionPerLevel, maxReduction);
        
        // 应用减伤，这里只执行一次计算，避免二次叠加
        // Apply damage reduction, calculated once here to avoid double dipping
        ...
        if (!target.level().isClientSide) {
            ...
            if (event.getSource().getEntity() instanceof LivingEntity attacker) {
                // 传递减伤百分比和当前层数作为参数
                // Pass reduction percentage and current level as arguments
                DebugCommand.sendDebugMessage(attacker, 
                    Component.translatable("debug.elementalcraft.reaction.steam", 
                        (int)(reductionRatio * 100), 
                        wetnessLevel)
                );
            }
        }
     ...
     * Reads radius, duration, and effects from config.
     */
    private static void spawnSteamCloud(LivingEntity target) {
        ...
        // 设置半径 (从配置读取) / Set Radius (Read from config)
        ...
        // 设置持续时间 (从配置读取) / Set Duration (Read from config)
        ...
        // 添加药水效果：失明 (持续时间从配置读取)
        // Add Potion Effect: Blindness (Duration from config)
