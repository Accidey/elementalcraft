...
public class ScorchedHandler {
    ...
    public static void onLivingTick(LivingEvent.LivingTickEvent event) {
        ...
        if (!data.contains(NBT_SCORCHED_TICKS)) {
            if (data.getBoolean(NBT_TEMP_SCORCH)) {
                ...
                if (handleFrozenToSteam(entity, data, tempStrength, "temp_scorch_frozen_steam", true)) {
                    clearTempScorch(entity);
                    return;
                }
        ...
        if (handleFrozenToSteam(entity, data, data.getInt(NBT_SCORCHED_SOURCE_FIRE_POWER), "scorch_frozen_steam", false)) {
            clearScorched(entity);
            return;
        }
    ...
    private static boolean handleFrozenToSteam(LivingEntity entity, CompoundTag data, int fireStrength, String debugKey, boolean isFromAura) {
        if (!FrostbiteHandler.isFrozen(entity)) return false;
        if (isFromAura && !ElementalThunderFrostReactionsConfig.frostbiteAuraScorchedSteamEnabled) return false;
