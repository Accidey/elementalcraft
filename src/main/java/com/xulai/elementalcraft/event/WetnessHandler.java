 ...
 * 负责处理潮湿系统的核心逻辑。
 * 包含：
 * 1. 动态衰减时间（级联降级）：使用配置文件定义的基础时间。
 * 2. 环境积累逻辑：雨中每 N 秒增加一层。
 * 3. 属性抗性变化：
 * - ▼ 雷霆/冰霜抗性：受到伤害增加（数值可配）。
 * - ▲ 赤焰抗性：受到伤害减少（数值可配）。
 * - ▲ 饱食度消耗：略微增加。
 ...
 * Includes:
 * 1. Dynamic decay time (cascading downgrade): Uses base time defined in config.
 * 2. Environmental accumulation logic: Gain one level every N seconds in rain.
...
public class WetnessHandler {
     ...
     * 监听生物受伤事件。
     * 处理抗性变化：雷霆/冰霜易伤，赤焰减伤。
     * 注意：此处的逻辑仅用于修改原版或其他模组造成的元素伤害（如果有）。
     * 本模组自身的 CombatEvents 已经集成了这部分逻辑，两者不会冲突。
     *
     * Listens to Living Hurt events.
     * Handles resistance changes: Vulnerability to Thunder/Frost, Resistance to Fire.
     * Note: This logic only modifies elemental damage from vanilla or other mods (if any).
     * The mod's own CombatEvents already integrates this logic, so there is no conflict.
     *
    ...
    private static void handleWetnessLogic(LivingEntity entity) {
        ...
        if (inWater) {
            ...
            if (currentLevel > 0) {
                ...
                // 计算当前层数应该持续的总时间
                // 公式：层数 * 基础时间 (例如配置为10: 4层=40s, 3层=30s)
                // Calculate total duration for current level
                // Formula: Level * Base Time (e.g., config=10: Lvl 4 = 40s)
