...
public class DebugCommand {
    ...
    public static void sendCombatLog(LivingEntity attacker, LivingEntity target, Entity directEntity,
        ...
        double increasePerLevel = ElementalReactionConfig.wetnessResistModifier;
        ...
        if (wetnessLevel > 0) {
            ...
            double otherBase = 1.0 + (wetnessLevel * increasePerLevel);
            ...
            // 判定：匹配 OtherBase -> 仅潮湿增伤
            else if (Math.abs(wetnessMult - otherBase) < 0.01) {
                calculatedBaseWetnessMult = otherBase;
            }
            // [新增] 兜底逻辑：如果反推失败但确实有数值变化，直接使用实际传入的 wetnessMult
            // [New] Fallback: If inference fails but value changed, use actual wetnessMult
     ...
     * Shows triggered steam type and level.
     *
     * @param attacker   攻击者 / Attacker
     * @param isHighHeat 是否为高温 / Whether high heat
     * @param level      蒸汽层数 / Steam Level
     */
     ...
     * Shows the change in wetness levels and fire power.
     *
     * @param entity        执行干燥的实体 / Entity performing drying
     * @param oldLevel      旧层数 / Old wetness level
     * @param newLevel      新层数 / New wetness level
     * @param removedLayers 移除的层数 / Layers removed
     * @param firePower     赤焰强度 / Fire power
     */
