...
public class DebugCommand {
     ...
     * Includes detailed data on physical damage, restraint floor mechanism, and wetness effects.
     *
     ...
     * @param wetnessMultiplier 潮湿伤害倍率 / Wetness damage multiplier
     */
    public static void sendCombatLog(LivingEntity attacker, LivingEntity target, Entity directEntity,
                                     ...
                                     int wetnessLevel, float wetnessMultiplier) {
        ...
        // 如果目标处于潮湿状态，追加潮湿信息
        // If target is affected by wetness, append wetness information
        if (wetnessLevel > 0) {
            // 使用翻译键： " [潮湿 Lv.%s | x%s]"
            // Use translation key
            logBuilder.append(Component.translatable("debug.elementalcraft.wetness_suffix", 
                wetnessLevel, 
                String.format("%.2f", wetnessMultiplier)
            ).withStyle(s -> s.withColor(0x00AAAA))); // Dark Aqua
        }
