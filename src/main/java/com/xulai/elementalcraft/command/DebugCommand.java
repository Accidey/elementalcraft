...
public class DebugCommand {
    ...
    /**
     * 发送详细的战斗伤害计算日志。
     * 格式参考：玩家1克制僵尸！造成10点伤害，物理5点+属性攻击5点（公式详情...）
     *
    ...
    public static void sendCombatLog(LivingEntity attacker, LivingEntity target, Entity directEntity,
        ...
        // e.g. "Dealt 10 damage, Phys 5 + Elem 5"
        MutableComponent overview = Component.translatable("debug.elementalcraft.damage_overview",
                String.format("%.1f", totalDamage),
                String.format("%.1f", physicalDamage),
                String.format("%.1f", finalElemDmg)
        ).withStyle(ChatFormatting.WHITE);

        // 3. 构建公式详情 (Formula Details)
        // Format: (Enhance * DmgMult * Restraint * Wetness - Resist * ResMult [+ Floor])
        
        MutableComponent formula = Component.literal(" (");

        // 3.1 攻击部分 (Attack Part)
        formula.append(Component.translatable("debug.elementalcraft.formula.enhance", String.format("%.1f", rawElemDmg)).withStyle(ChatFormatting.GOLD));
        formula.append(" x ").append(Component.translatable("debug.elementalcraft.formula.dmg_mult", String.format("%.1f", globalDmgMult)).withStyle(ChatFormatting.GRAY));
        formula.append(" x ").append(Component.translatable("debug.elementalcraft.formula.restraint", String.format("%.1f", restraintMult)).withStyle(ChatFormatting.LIGHT_PURPLE));
        
        // 潮湿修正显示 (Wetness)
        if (wetnessLevel > 0) {
            if (wetnessMult != 1.0f) {
                // 如果潮湿确实影响了数值（火/雷/冰）
                formula.append(" x ").append(Component.translatable("debug.elementalcraft.formula.wetness", wetnessLevel, String.format("%.1f", wetnessMult)).withStyle(ChatFormatting.AQUA));
            } else {
                // 自然属性（不受影响）
        ...
        formula.append(Component.translatable("debug.elementalcraft.formula.resist", String.format("%.1f", rawResistReduct)).withStyle(ChatFormatting.BLUE));
        formula.append(" x ").append(Component.translatable("debug.elementalcraft.formula.res_mult", String.format("%.1f", globalResistMult)).withStyle(ChatFormatting.GRAY));
    ...
    public static void sendExhaustionLog(Player player, float baseDelta, float wetnessDelta, int wetnessLevel) {
        ...
        player.displayClientMessage(msg, true); // true = Action Bar (显示在物品栏上方，避免刷屏聊天栏)
    }
}