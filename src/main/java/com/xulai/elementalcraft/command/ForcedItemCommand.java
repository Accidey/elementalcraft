 ...
 * <p>
 * 中文说明：
 * 负责注册和管理 /elementalcraft item 命令。
 * 该命令允许拥有 OP 权限的玩家通过手持物品，将特定的元素属性（武器攻击属性或防具的增强/抗性属性）绑定到该物品类型上。
 * 配置更改会直接写入文件，并触发 ItemHelper 的缓存清理以实现热重载。
 * <p>
 * English Description:
 * Responsible for registering and managing the /elementalcraft item command.
 ...
 * Configuration changes are written directly to the file and trigger a cache clear in ItemHelper for hot-reloading.
 */
...
public class ForcedItemCommand {
    ...
    /**
     * 注册 /elementalcraft item 命令及其子命令。
     * <p>
     * Registers the /elementalcraft item command and its subcommands.
     *
