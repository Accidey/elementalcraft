...
public class ElementAttackEnchantment extends Enchantment {
    ...
    /**
     * 附魔兼容性检查。
     * 1. 检查配置：若开启互斥，则元素攻击附魔与原版火焰附加 (Fire Aspect) 互斥。
     * 2. 检查同类：不同元素类型的攻击附魔互斥（如赤焰与冰霜不能共存），同类型兼容。
     *
     * Enchantment compatibility check.
     * 1. Config Check: If exclusivity is enabled, Element Attack is mutually exclusive with Vanilla Fire Aspect.
    ...
    public boolean checkCompatibility(Enchantment other) {
        // Rule 1: Exclusivity with Vanilla Fire Aspect (Configurable) / 规则1：与原版火焰附加互斥（可配置）
        if (ElementalReactionConfig.enchantFireAspectExclusive && other == Enchantments.FIRE_ASPECT) {
            return false;
        }

        // Rule 2: Exclusivity between different Element Types / 规则2：不同元素类型之间互斥
