...
public class ElementEnhancementEnchantment extends Enchantment {
    ...
    /**
     * 构造函数，创建对应元素的强化附魔。
     * 使用 ModEnchantments 中统一定义的 STRICT_ARMOR 类别。
     * <p>
     * Constructor to create the enhancement enchantment for the corresponding element.
     * Uses the STRICT_ARMOR category centrally defined in ModEnchantments.
     *
     ...
     * Added protection check against division by zero.
     */
    @Override
    public int getMaxLevel() {
        int perLevel = ElementalConfig.getStrengthPerLevel();
        if (perLevel <= 0) return 1; // 防止配置错误导致的除零异常 / Prevent division by zero due to config error
