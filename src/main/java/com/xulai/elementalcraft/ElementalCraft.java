...
public class ElementalCraft {
    ...
    public ElementalCraft() {
        ...
        // 注册强制物品配置文件
        // Register forced item config
        ForcedItemConfig.register();
        
        // [新增] 注册元素反应配置文件
        // [New] Register Elemental Reaction Config
        ...
        // [新增] 注册药水效果
        // [New] Register Potion Effects
        ModMobEffects.register(modEventBus);

        // 监听通用设置事件（用于初始化缓存）
        // Listen for common setup event (to initialize cache)
        ...
        // 监听配置加载事件（首次加载）
        // Listen for config load events (first load)
        ...
        // 注意：PlayerTrackEvents 已被拆分为逻辑类，这里只注册入口
        // Note: PlayerTrackEvents has been refactored, registering entry point only
    ...
    /**
     * 通用初始化阶段。
     * 确保配置缓存已初始化，防止游戏早期逻辑除以零。
     *
     * Common setup phase.
     * Ensures config cache is initialized to prevent divide-by-zero in early game logic.
     */
    private void commonSetup(final FMLCommonSetupEvent event) {
        ElementalConfig.refreshCache();
        // [新增] 刷新反应配置缓存
        // [New] Refresh Reaction Config Cache
     ...
     * 当配置文件首次加载时触发。
     *
     * Triggered when config file is loaded for the first time.
     */
    public void onConfigLoad(ModConfigEvent.Loading event) {
        ...
        // [新增] 监听反应配置加载
        // [New] Listen for Reaction Config Load
    ...
    public void onConfigReload(ModConfigEvent.Reloading event) {
        if (event.getConfig().getSpec() == ElementalConfig.SPEC) {
            // [CRITICAL] 刷新静态缓存 / Refresh static cache
            ElementalConfig.refreshCache();
        ...
        // [新增] 监听反应配置重载
        // [New] Listen for Reaction Config Reload
        if (event.getConfig().getSpec() == ElementalReactionConfig.SPEC) {
            ElementalReactionConfig.refreshCache();
    ...
    public void onAddReloadListeners(AddReloadListenerEvent event) {
        event.addListener(new ResourceManagerReloadListener() {
            ...
            public void onResourceManagerReload(ResourceManager resourceManager) {
                ElementalConfig.refreshCache(); // [CRITICAL]
                
                // [新增] 刷新反应配置
                // [New] Refresh Reaction Config
                ElementalReactionConfig.refreshCache();
                
                CustomBiomeBias.clearCache();
                ForcedAttributeHelper.clearCache();
                ForcedItemHelper.clearCache();
                LOGGER.info("[ElementalCraft] /reload detected, all caches forced refresh.");
            }
        });
    }
}