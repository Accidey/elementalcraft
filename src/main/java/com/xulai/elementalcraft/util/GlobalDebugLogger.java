...
// import net.minecraftforge.event.entity.living.LivingHealEvent; // 移除引用
...
public class GlobalDebugLogger {
    ...
    public static void onEntityJoinLevel(EntityJoinLevelEvent event) {
        ...
        if (!isInteresting) return; // 普通生物不刷屏
    ...
    public static void onLivingDamage(LivingDamageEvent event) {
        ...
        if (event.getSource().getEntity() instanceof LivingEntity attacker) {
            ...
            // 这里只记录判定，不记录具体回血，具体回血应该在 NatureHandler 触发的地方记录
    ...
    /**
     * [已删除] onLivingHeal
     * 原因：自然回血也会触发此事件，且数额通常为 1.0，导致无法区分是否为 Mod 效果，造成误报刷屏。
     * 如果需要调试回血，请在 NatureReactionHandler 中执行 heal() 的地方添加日志。
     */
    /*
    @SubscribeEvent
