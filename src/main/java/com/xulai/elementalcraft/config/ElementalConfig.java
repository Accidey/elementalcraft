...
public final class ElementalConfig {
    ...
    static {
        ...
        RESTRAINT_MULTIPLIER = BUILDER
                .comment("Damage multiplier when target is restrained",
                        "克制伤害倍率")
                .defineInRange("restraint_multiplier", 1.5, 0.1, 10.0);

        WEAK_MULTIPLIER = BUILDER
                .comment("Damage multiplier when target resists the element",
                        "被克制伤害倍率")
    ...
    /**
     * 手动刷新缓存。
     * 必须在 ModConfigEvent 事件中调用，以确保静态字段同步最新配置值。
     * <p>
     * Manually refreshes the cache.
     * Must be called during the ModConfigEvent event to ensure static fields are synced with the latest config values.
     */
    ...
    public static int getStrengthPerHalfDamage() {
        return Math.max(1, strengthPerHalfDamage);
    }

    public static int getResistPerHalfReduction() {
        return Math.max(1, resistPerHalfReduction);
    }
    ...
    public static int getStrengthPerLevel() {
        return Math.max(1, strengthPerLevel);
    }

    public static int getResistPerLevel() {
        return Math.max(1, resistPerLevel);
    }
     ...
     *
     * @param attackElement 攻击元素 / Attack element
     ...
     *
     * @param c1       0-20% 区间概率 / Probability for 0-20% range
     * @param c2       30-50% 区间概率 / Probability for 30-50% range
     * @param c3       60-80% 区间概率 / Probability for 60-80% range
     * @param c4       90-100% 区间概率 / Probability for 90-100% range
     * @param maxValue 最大值上限 / Maximum value cap
     * @return 随机生成的数值 / Randomly generated value
     */
     ...
     * 随机生成怪物的强化属性值。
     * <p>
     * Randomly generates the strength attribute value for a monster.
     *
     * @return 强化值 / Strength value
     */
     ...
     * 随机生成怪物的抗性属性值。
     * <p>
     * Randomly generates the resistance attribute value for a monster.
     *
     * @return 抗性值 / Resistance value
     */
