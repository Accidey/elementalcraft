...
public final class ElementalConfig {
    ...
    private static int rollDynamicValue(double c1, double c2, double c3, double c4, int maxValue) {
        ...
        double s1 = c1, s2 = s1 + c2, s3 = s2 + c3;

        int min, max;
        if (roll < s1) { min = 10; max = (int)(cap * 0.20); }
        else if (roll < s2) { min = (int)(cap * 0.30); max = (int)(cap * 0.50); }
        else if (roll < s3) { min = (int)(cap * 0.60); max = (int)(cap * 0.80); }
        else { min = (int)(cap * 0.90); max = cap; }
    ...
    public static int rollMonsterStrength() {
        return rollDynamicValue(chance0_20, chance30_50, chance60_80, chance90_100, maxStatCap);
    }
