 ...
 * 已完全支持热重载（通过静态缓存和 refreshCache 方法）。
 * 包含了防止除以零的安全检查。
 *
 * English description:
 ...
 * Fully supports hot-reloading (via static caching and refreshCache method).
 * Includes safety checks to prevent divide-by-zero errors.
 */
public final class ElementalConfig {
    ...
    // Initialize with safe defaults to prevent divide-by-zero before refreshCache
    ...
    // 概率缓存
    public static double chance0_20, chance20_40, chance40_60, chance60_80, chance80_100;

    /**
     * 手动刷新缓存。
     * 必须在 ModConfigEvent 事件 或 ConfigAutoSync 检测到文件变化时调用。
     * 该方法强制从配置对象中重新读取值并更新到静态缓存字段，确保游戏逻辑使用的是最新配置。
     *
     * Refreshes local static cache.
     * Must be called during ModConfigEvent or when ConfigAutoSync detects file changes.
     * Forces retrieval of values from config object to update static cache fields, ensuring game logic uses the latest config.
     */
    public static void refreshCache() {
        // Reload primitive values from ConfigSpec
        // 从 ConfigSpec 重新加载基本数值
        ...
        // Ensure values are clamped/normalized as needed and NOT ZERO
        // 确保数值按需被限制/标准化，且不为0
        maxStatCap = Math.max(10, (MAX_STAT_CAP.get() / 10) * 10);
        strengthPerLevel = Math.max(1, STRENGTH_PER_LEVEL.get()); // Prevent 0
        resistPerLevel = Math.max(1, RESIST_PER_LEVEL.get());     // Prevent 0
        strengthPerHalfDamage = Math.max(10, (STRENGTH_PER_HALF_DAMAGE.get() / 10) * 10);
        resistPerHalfReduction = Math.max(10, (RESIST_PER_HALF_REDUCTION.get() / 10) * 10);

        // Reload probabilities
        // 重新加载概率
        chance0_20 = CHANCE_0_20.get();
        chance20_40 = CHANCE_20_40.get();
        chance40_60 = CHANCE_40_60.get();
        chance60_80 = CHANCE_60_80.get();
        chance80_100 = CHANCE_80_100.get();
    }

    // ==================== Utility Methods / 工具方法 ====================
    // Modified to use static cache fields instead of .get()
    // 已修改为使用静态缓存字段而非 .get()
     ...
     * 使用缓存值以确保热重载生效。
     *
     * Calculates the restraint multiplier based on attacker and target elements.
     * Returns 1.0 if target element is NONE.
     * Uses cached values to ensure hot reload works.
     *
     * @param attackElement 攻击方元素 / Attacker element
     * @param targetElement 目标元素 / Target element
     * @return 克制倍率 / Restraint multiplier
     */
    public static float getRestraintMultiplier(ElementType attackElement, ElementType targetElement) {
        ...
        // List config doesn't need aggressive caching like primitives, .get() is usually fine or managed by Forge
        // List 配置通常不需要像基本类型那样激进缓存，.get() 通常没问题
        List<? extends String> rules = ELEMENT_RESTRAINTS.get();

        if (rules.contains(relation)) {
            return (float) restraintMultiplier; // Use cached value / 使用缓存值
        }

        String reverse = targetElement.getId() + "->" + attackElement.getId();
        if (rules.contains(reverse)) {
            return (float) weakMultiplier; // Use cached value / 使用缓存值
        }
     ...
     * Uses cached probability values.
     */
    ...
    public static int rollMonsterStrength() {
        // Pass cached static fields
        // 传入缓存的静态字段
        return rollDynamicValue(chance0_20, chance20_40, chance40_60, chance60_80, chance80_100, maxStatCap);
    }
