 ...
 * Update: Added step and delay configurations required for low-heat steam condensation logic.
 */
public class ElementalReactionConfig {
    ...
    // Condensation Logic / 冷凝逻辑
    public static final ForgeConfigSpec.IntValue STEAM_CONDENSATION_INTERVAL; // Deprecated but kept for compatibility or other uses
    public static final ForgeConfigSpec.IntValue STEAM_CONDENSATION_STEP_FIRE; // 新增：赤焰分层步长
    public static final ForgeConfigSpec.IntValue STEAM_CONDENSATION_STEP_FROST; // 新增：冰霜分层步长
    public static final ForgeConfigSpec.IntValue STEAM_CONDENSATION_DELAY; // 新增：获得潮湿延迟

    // Low-Heat Duration / 低温持续时间 (New Configs)
    ...
    static {
        ...
        STEAM_CONDENSATION_INTERVAL = BUILDER
                .comment("Interval (in Ticks) to check condensation logic (Deprecated but kept).",
                        "冷凝逻辑检查间隔（已废弃但保留兼容性）。")
                .defineInRange("steam_condensation_interval", 20, 1, 1200);
        
        // --- New Configs for Condensation ---
        STEAM_CONDENSATION_STEP_FIRE = BUILDER
                .comment("Fire Enhancement points required per level of Low-Heat Steam cloud (Frost attack Fire).",
                        "低温蒸汽云（冰打火）：每增加 1 个等级所需的目标赤焰强化点数步长。",
        ...
        STEAM_CONDENSATION_STEP_FROST = BUILDER
                .comment("Frost Enhancement points required per level of High-Heat Steam cloud (Fire attack Frost).",
                        "高温蒸汽云（火打冰）：每增加 1 个等级所需的目标冰霜强化点数步长。",
        ...
        STEAM_CONDENSATION_DELAY = BUILDER
                .comment("Time (in Ticks) an entity must stay inside Low-Heat Steam to gain Wetness.",
                        "实体必须在低温蒸汽中停留多久（Tick）才会获得 1 层潮湿效果。",
                        "默认 100 Tick = 5 秒。")
    ...
    public static int steamCondensationStepFire = 20; // Cache
    public static int steamCondensationStepFrost = 20; // Cache
    public static int steamCondensationDelay = 100; // Cache
    public static int steamCondensationDurationBase = 100; // Cache New
    public static int steamCondensationDurationPerLevel = 20; // Cache New
    ...
    public static void refreshCache() {
        ...
        steamCondensationStepFire = STEAM_CONDENSATION_STEP_FIRE.get(); // Refresh
        steamCondensationStepFrost = STEAM_CONDENSATION_STEP_FROST.get(); // Refresh
        steamCondensationDelay = STEAM_CONDENSATION_DELAY.get(); // Refresh
        steamCondensationDurationBase = STEAM_CONDENSATION_DURATION_BASE.get(); // Refresh New
        steamCondensationDurationPerLevel = STEAM_CONDENSATION_DURATION_PER_LEVEL.get(); // Refresh New
