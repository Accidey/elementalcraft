 ...
 * 主要配置板块包括：潮湿机制、孢子感染、传染扩散、自然系反应、赤焰系反应（含连锁爆炸）以及蒸汽反应。
 * <p>
 ...
 * Main configuration sections include: Wetness Mechanics, Spore Infection, Contagion Spread, Nature Reactions, Fire Reactions (w/ Chain Reaction), and Steam Reactions.
 */
public class ElementalReactionConfig {
    public static final ForgeConfigSpec SPEC;

    /**
     * Wetness System Configuration
     * 潮湿系统配置
     * This section defines all parameters related to the wetness status effect, including maximum stacks, damage reductions, gain/decay rates, and immunities.
     * 本节定义了与潮湿状态效果相关的所有参数，包括最大叠加层数、伤害减免、增减速率以及免疫设置。
     */
    ...
    /**
     * Spore System Configuration
     * 孢子系统配置
     * This section configures the spore infection mechanics, including maximum stacks, damage effects, resistances, vulnerabilities, and immunities.
     * 本节配置了孢子感染机制，包括最大叠加层数、伤害效果、抗性、易伤以及免疫设置。
     */
    ...
    /**
     * Contagion System Configuration
     * 传染系统配置
     * This section handles the spore contagion spread mechanics, including check intervals, radii, intensity, and interactions with wetness.
     * 本节处理孢子传染扩散机制，包括检测间隔、半径、强度以及与潮湿的互动。
     */
    ...
    /**
     * Nature Reaction Configuration
     * 自然反应配置
     * This section defines parameters for nature-based reactions, including dynamic parasitism, parasitic drain, and wildfire ejection.
     * 本节定义了基于自然的反应参数，包括动态寄生、寄生吸取以及野火喷射。
     */
     ...
     * 赤焰反应配置
     * This section configures fire-based reactions, including toxic blast triggers, damage scaling, chain reactions, and defense caps.
     * 本节配置了基于赤焰的反应，包括毒火爆燃触发、伤害缩放、连锁反应以及防御上限。
     */
    ...
    /**
     * Steam Reaction Configuration
     * 蒸汽反应配置
     * This section defines all aspects of steam reactions, including enabling the feature, heat levels, cloud properties, condensation, scalding damage, triggers, and defenses.
     * 本节定义了蒸汽反应的所有方面，包括启用功能、热量等级、云属性、凝结、烫伤伤害、触发以及防御。
     */
    ...
    /**
     * Scorched Mechanic Configuration
     * 灼烧机制配置
     * This section configures the scorched effect, a persistent burning mechanic, including triggers, chances, durations, damages, and immunities.
     * 本节配置了灼烧效果，一种持久燃烧机制，包括触发、概率、持续时间、伤害以及免疫。
     */
    ...
    static {
        ...
        WILDFIRE_KNOCKBACK = BUILDER
                .comment("反击造成的击退力度。",
                        "Knockback strength of the counter-attack.")
        ...
        BLAST_WEAK_IGNITE_MULT = BUILDER
                .comment("低层数（未爆炸）时的引燃伤害倍率。",
                        "Ignite damage multiplier for low stacks (no explosion).")
                .defineInRange("blast_weak_ignite_mult", 1.5, 1.0, 100.0);
        BLAST_BASE_DAMAGE = BUILDER
                .comment("3层孢子引爆时的基础爆炸伤害。",
                        "Base explosion damage when detonating 3 stacks of spores.")
        ...
        BLAST_GROWTH_DAMAGE = BUILDER
                .comment("超过3层后，每多一层孢子增加的爆炸伤害。",
                        "Bonus explosion damage per extra spore stack above 3.")
        ...
        BLAST_GROWTH_RANGE = BUILDER
                .comment("超过3层后，每多一层孢子增加的爆炸半径（格）。",
                        "Bonus explosion radius per extra spore stack above 3.")
                .defineInRange("blast_growth_range", 1.0, 0.0, 5.0);
        BLAST_SCORCH_BASE = BUILDER
                .comment("弱效引燃（<3层）造成的灼烧持续时间（秒）。",
                        "Duration (seconds) of weak scorching applied by low stacks.")
                .defineInRange("blast_scorch_base", 3.0, 0.0, 6000.0);
        BLAST_BASE_SCORCH_TIME = BUILDER
                .comment("成功爆炸（3层）后造成的灼烧持续时间（秒）。",
                        "Duration (seconds) of scorching applied by a successful explosion.")
                .defineInRange("blast_base_scorch_time", 3.0, 0.0, 6000.0);
        BLAST_GROWTH_SCORCH_TIME = BUILDER
                .comment("超过3层后，每多一层孢子增加的灼烧时间（秒）。",
                        "Bonus scorch time (seconds) per extra spore stack above 3.")
     ...
     * 静态缓存变量
     * These variables cache the configuration values for fast runtime access, avoiding repeated calls to ForgeConfigSpec's get() method.
     * 这些变量缓存配置值，用于运行时快速访问，避免重复调用 ForgeConfigSpec 的 get() 方法。
     */
     ...
     * 注册配置文件。
     * This method registers the config with Forge's mod loading context using the common type and a specific file name.
     * 此方法使用通用类型和特定文件名将配置注册到 Forge 的模组加载上下文中。
     */
     ...
     * 刷新配置缓存。
     * This method reads values from the config file into static cache variables for efficient runtime access.
     * 此方法将配置文件中的值读取到静态缓存变量中，以提高运行时访问效率。
     */
