...
public class ElementalFireNatureReactionsConfig {
    ...
    public static final ForgeConfigSpec.DoubleValue WILDFIRE_HEALTH_RECOVERY_THRESHOLD;
    ...
    STEAM_SCALDING_MULTIPLIER_FROST = BUILDER
            .comment("冰霜属性生物受到高温蒸汽烫伤伤害的**乘法倍率**。",
                    ...
                    "Default: 1.5 / 默认：1.5")
            .defineInRange("steam_scalding_multiplier_frost", 1.5, 0.0, 1000.0);
    BUILDER.comment(" ");
   ...
   SCORCHED_FROST_DMG_MULTIPLIER = BUILDER
    ...
    BLAST_BASE_RANGE = BUILDER
            .comment("基础爆炸半径（格）。",
                    ...
                    "Default: 3.0 / 默认：3.0")
            .defineInRange("blast_base_range", 3.0, 0.5, 1000.0);
    BUILDER.comment(" ");
    ...
    WILDFIRE_BLOOD_THRESHOLD = BUILDER
            .comment("触发自然反制所需的血量阈值（百分比）。生物血量低于此比例时受伤才会触发。设为0则关闭自然反制。",
                    "Health threshold to trigger Nature Counter. Entity must be below this health percentage when taking damage. Set to 0 to disable.",
                    "Default: 0.5 / 默认：0.5")
            .defineInRange("wildfire_blood_threshold", 0.5, 0.0, 1.0);
    BUILDER.comment(" ");

    WILDFIRE_HEALTH_RECOVERY_THRESHOLD = BUILDER
            .comment("触发自然反制后，生命值需要恢复到多少比例才能再次触发。例如0.8表示生命值恢复到80%后才可再次触发。",
                     "Health recovery threshold to re-trigger Nature Counter. Entity must heal back to this percentage to be eligible again. E.g., 0.8 = 80%.",
                     "Default: 0.8 / 默认：0.8")
            .defineInRange("wildfire_health_recovery_threshold", 0.8, 0.0, 1.0);
    BUILDER.comment(" ");
