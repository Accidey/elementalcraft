...
public final class ElementalThunderFrostReactionsConfig {
    ...
    public static final ForgeConfigSpec.DoubleValue FROST_COUNTER_BASE_CHANCE;
    public static final ForgeConfigSpec.DoubleValue FROST_COUNTER_STATIC_STACK_BONUS;
    public static final ForgeConfigSpec.IntValue FROST_COUNTER_COOLDOWN_TICKS;
    ...
    static {
        ...
        BUILDER.comment("冰霜反制配置 - Frost Counter",
                        "当冰霜属性目标身上有静电效果时，静电周期性伤害有概率触发冰霜反制（冰暴）。",
                        "冰暴以目标为中心生成扩大的霜冻环，对范围内生物施加冻结效果。",
                        "目标若持续留在冰暴范围内，冻结层数会按间隔持续累加，光环消失后冻结按倒计时自然结束。",
                        "When a Frost-attribute entity with Static Shock takes periodic damage, there is a chance to trigger Frost Counter (Ice Burst).",
                        "Ice Burst creates an expanding frost ring centered on the target, applying Freeze to entities in range.",
                        "Entities that stay inside will accumulate more Freeze stacks over time. Freeze expires naturally after the ring disappears.")
                .push("frost_counter");
        ...
        FROST_COUNTER_BASE_CHANCE = BUILDER
                .comment("冰霜反制的基础触发概率。",
                         "Base trigger chance for Frost Counter.",
                         "Default: 0.5 (50%) / 默认：0.5（50%）")
                .defineInRange("base_chance", 0.5, 0.0, 1.0);

        BUILDER.comment(" ");

        FROST_COUNTER_STATIC_STACK_BONUS = BUILDER
                .comment("目标每层静电增加的额外触发概率。总概率 = base_chance + static_stacks × 此值。",
                         "Additional chance per Static Shock stack on the target. Total = base_chance + static_stacks × this.",
                         "Default: 0.10 (10% per stack) / 默认：0.10（每层10%）")
                .defineInRange("static_stack_bonus", 0.10, 0.0, 1.0);

        BUILDER.comment(" ");

        FROST_COUNTER_COOLDOWN_TICKS = BUILDER
                .comment("冰霜反制触发后的冷却时间（刻）。冷却期间不会再次触发。20刻 = 1秒。",
                         "Cooldown ticks after Frost Counter triggers. Cannot trigger again during cooldown. 20 ticks = 1 second.",
                         "Default: 300 (15 seconds) / 默认：300（15秒）")
                .defineInRange("cooldown_ticks", 300, 1, 72000);
        ...
        THUNDER_BREAK_FREEZE_CHANCE = BUILDER
                .comment("每层静电解除冰冻的概率。总概率 = 层数 × 此值。0 = 关闭。",
                         ...
                         "Default: 0.05 (5% per stack) / 默认：0.05（每层5%）")
    ...
    public static double frostCounterBaseChance;
    public static double frostCounterStaticStackBonus;
    public static int frostCounterCooldownTicks;
    ...
    public static void refreshCache() {
        ...
        frostCounterBaseChance = FROST_COUNTER_BASE_CHANCE.get();
        frostCounterStaticStackBonus = FROST_COUNTER_STATIC_STACK_BONUS.get();
        frostCounterCooldownTicks = FROST_COUNTER_COOLDOWN_TICKS.get();
