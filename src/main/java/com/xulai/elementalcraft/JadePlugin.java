 ...
 * 强化/抗性数值显示上限问题，现在会正确显示实际总点数（不受单件上限限制）。
 * 攻击属性检查主手和副手武器。
 *
 * English description:
 ...
 * Enhancement/resistance value display cap issue; now correctly shows actual total points (not limited by per-piece cap).
 * Attack element checks both main hand and off hand.
 */
@WailaPlugin
public class JadePlugin implements IWailaPlugin {
    ...
    private enum Provider implements IEntityComponentProvider {
         ...
         * 显示顺序：攻击属性 > 属性强化 > 属性抗性。
         * 强化和抗性显示实际总点数（所有装备累加），不再受单件上限影响。
         * 攻击属性检查主手和副手武器，只要任意一只手有攻击附魔即显示。
         *
         * Appends elemental attribute information to the Jade tooltip.
         * Display order: attack element > enhancement > resistance.
         * Enhancement and resistance now show actual total points (sum across all equipment), not capped by per-piece limit.
         * Attack element checks both main hand and off hand weapons.
         *
        ...
        public void appendTooltip(ITooltip tooltip, EntityAccessor accessor, IPluginConfig config) {
            ...
            if (attack == ElementType.NONE || attack == null) {
