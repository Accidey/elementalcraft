...
public class ModCommands {
    ...
    /**
     * 在服务器命令注册事件中手动注册部分命令。
     * DebugCommand 和 BiomeBiasCommand 使用传统方式通过 dispatcher.register() 注册，因此需要在这里手动调用其 register 方法。
     * 其他命令（如 BlacklistCommand、ForcedItemCommand、ForcedRemoveCommand 等）已通过各自类中的 @SubscribeEvent 自动注册，无需在此重复。
     *
     * Manually register certain commands during the server command registration event.
     * DebugCommand and BiomeBiasCommand use the traditional dispatcher.register() approach, so their register methods must be called here.
     * Other commands (e.g., BlacklistCommand, ForcedItemCommand, ForcedRemoveCommand) are automatically registered via @SubscribeEvent in their own classes and do not need to be repeated here.
     *
    ...
    public static void registerCommands(RegisterCommandsEvent event) {
        ...
        // 注意：其他命令已通过各自类中的 @SubscribeEvent 自动注册，无需在此处添加
        // Note: Other commands are automatically registered via @SubscribeEvent in their own classes and do not need to be added here
    }
}