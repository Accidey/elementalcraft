...
public class FrostVisuals {
    ...
    private static final List<IceRunePendingParticle> PENDING_RUNE_PARTICLES = new ArrayList<>();
    private static final Map<UUID, Integer> ENTITY_DEATH_TICKS = new HashMap<>();

    private record IceRunePendingParticle(
            ServerLevel level,
            UUID entityId,
            double originX,
            double originY,
        ...
        private static void playFrostMeleeSwing(LivingEntity entity, int tier) {
            ...
            for (int i = 0; i <= particleCount; i++) {
                ...
                if (tier >= 2 && RANDOM.nextFloat() < ElementalVisualConfig.frostMeleeGlowChanceTier2) {
                    level.addParticle(ModParticles.FROST_SNOWFLAKE.get(), pos.x, pos.y, pos.z, 0, 0, 0);
                }
                ...
                if (tier >= 3) {
                    ...
                    if (RANDOM.nextFloat() < ElementalVisualConfig.frostMeleeMistChanceTier3) {
                        level.addParticle(ParticleTypes.CLOUD, pos.x, pos.y, pos.z, 0, 0.02, 0);
                    }
                }
            ...
            if (tier >= 4 && ElementalVisualConfig.frostMeleeMistLineEnabled && startPos != null && endPos != null) {
                ...
                for (int s = 0; s <= steps; s++) {
                    ...
                    if (s % 3 == 0) {
                        level.addParticle(ParticleTypes.CLOUD, x, y, z, 0, 0.03, 0);
                    }
                }
            }
        }
    ...
    public static void onServerTick(TickEvent.ServerTickEvent event) {
        ...
        if (ElementalVisualConfig.frostIceRuneRingEnabled) {
            processPendingRuneTasks();
        }
    ...
    private static void spawnFrostHelixTrail(ServerLevel level, Projectile p, int tier) {
        ...
        if (ElementalVisualConfig.frostRangedInnerCoreEnabled) {
            ...
            for (int h = 0; h < activatedHelicesInner; h++) {
                ...
                level.sendParticles(ModParticles.FROST_SNOWFLAKE.get(),
    ...
    public static void onLivingDeath(LivingDeathEvent event) {
        ...
        int deathGameTime = (int) (event.getEntity().level().getGameTime() % Integer.MAX_VALUE);

        synchronized (ENTITY_DEATH_TICKS) {
            ENTITY_DEATH_TICKS.put(entityId, deathGameTime);
        }
    }
    ...
    private static void playFrostImpact(Entity target, int tier) {
        ...
        if (ElementalVisualConfig.frostIceRuneRingEnabled) {
            spawnFrostIceRuneRing(serverLevel, target, tier);
        }
    }

    private static void spawnFrostIceRuneRing(ServerLevel level, Entity target, int tier) {
        double centerX = target.getX();
        double groundY = target.getY();
        double centerZ = target.getZ();

        double radius = ElementalVisualConfig.frostIceRuneRingRadius;
        int particleCount = ElementalVisualConfig.frostIceRuneRingTotalParticles;
        int gameTime = (int) (level.getGameTime() % Integer.MAX_VALUE);
        UUID entityId = target.getUUID();

        synchronized (PENDING_RUNE_PARTICLES) {
            ENTITY_DEATH_TICKS.remove(entityId);
        }

        for (int i = 0; i < particleCount; i++) {
            int angleDeg = i * 10;
            int delay = RANDOM.nextInt(ElementalVisualConfig.frostIceRuneRingSpawnDurationTicks + 1);
            int spawnTick = gameTime + delay;

            synchronized (PENDING_RUNE_PARTICLES) {
                PENDING_RUNE_PARTICLES.add(new IceRunePendingParticle(
                        level, entityId, centerX, groundY, centerZ, angleDeg, spawnTick));
            }
        }
    }

    private static void processPendingRuneTasks() {
        if (PENDING_RUNE_PARTICLES.isEmpty()) return;

        double radius = ElementalVisualConfig.frostIceRuneRingRadius;

        synchronized (PENDING_RUNE_PARTICLES) {
            Iterator<IceRunePendingParticle> it = PENDING_RUNE_PARTICLES.iterator();
            while (it.hasNext()) {
                IceRunePendingParticle pending = it.next();
                long currentGameTime = pending.level().getGameTime();
                UUID eid = pending.entityId();
                ServerLevel level = pending.level();

                boolean entityDead = false;
                synchronized (ENTITY_DEATH_TICKS) {
                    Integer deathTickObj = ENTITY_DEATH_TICKS.get(eid);
                    if (deathTickObj != null) {
                        if (currentGameTime > deathTickObj + ElementalVisualConfig.frostIceRuneRingDeathGraceTicks) {
                            it.remove();
                            continue;
                        }
                        entityDead = true;
                ...
                if (currentGameTime < pending.spawnTick()) continue;

                Entity targetEntity = level.getEntity(eid);

                double px, pz, py;
                if (targetEntity != null && targetEntity.isAlive()) {
                    px = targetEntity.getX() + Math.cos(Math.toRadians(pending.angleDeg())) * radius;
                    pz = targetEntity.getZ() + Math.sin(Math.toRadians(pending.angleDeg())) * radius;
                    py = targetEntity.getY();
                } else if (entityDead && targetEntity != null) {
                    px = pending.originX() + Math.cos(Math.toRadians(pending.angleDeg())) * radius;
                    pz = pending.originZ() + Math.sin(Math.toRadians(pending.angleDeg())) * radius;
                    py = pending.originY();
                } else {
                    it.remove();
                    continue;
