...
/**
 * ForcedItemCommand 类负责注册和管理 /elementalcraft forceditem 命令。
 * 该命令允许 OP 玩家通过手持物品为特定物品强制绑定元素属性（武器攻击属性或装备强化/抗性属性），并写入配置文件。
 * 支持热重载，配置生效后怪物装备或玩家获得该物品时会自动应用属性。
 *
 * ForcedItemCommand class is responsible for registering and handling the /elementalcraft forceditem command.
 ...
 * and writes the configuration to file. Supports hot-reloading; once configured, attributes are automatically applied when mobs equip or players obtain the item.
 */
...
public class ForcedItemCommand {
    ...
    private static int addWeaponForced(CommandContext<CommandSourceStack> ctx, String elementRaw) {
        ...
        List<String> list = new java.util.ArrayList<>(ForcedItemConfig.FORCED_WEAPONS.get());
    ...
    private static int addArmorForced(CommandContext<CommandSourceStack> ctx,
        ...
        List<String> list = new java.util.ArrayList<>(ForcedItemConfig.FORCED_ARMOR.get());
        String targetPrefix = itemId + ",";
        list.removeIf(s -> normalizeLine(s).startsWith(targetPrefix));
        list.add(newLine);
